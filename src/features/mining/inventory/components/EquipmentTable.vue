...
<template>
   <div class="equipment-table-wrap">
      <table class="equipment-table">
         ...
         <tbody>
            <tr v-for="item in equipment" :key="item.id" :class="{ 'equipment-table__row--selected': selectedId === item.id }" tabindex="0" @click="emit('select', item.id)" @keydown.enter="emit('select', item.id)">
