...
.equipment-card {
   display:flex;min-height:368px;flex-direction:column;gap:12px;padding:13px;border:1px solid var(--color-border);border-radius:var(--radius-sm);outline:none;background:var(--color-bg-tertiary);cursor:pointer;transition:border-color var(--duration-fast),box-shadow var(--duration-fast),background var(--duration-fast);
   ...
   &__category { color:var(--color-text-secondary);font-size:8px;font-weight:var(--font-bold);letter-spacing:.08em;text-transform:uppercase; }
   &__identity { min-height:40px; }
   &__identity h3 { font-size:var(--text-sm); }
   &__identity span { color:var(--color-text-muted);font-size:9px; }
   ...
   &__metrics div { display:grid;gap:2px;padding:8px;border-right:1px solid rgba(255,255,255,.05); }
   &__metrics div:last-child{border-right:0;}
   &__metrics dt { color:var(--color-text-secondary);font-size:8px; }
   &__metrics dd { overflow:hidden;font-size:9px;font-weight:var(--font-semibold);text-overflow:ellipsis;white-space:nowrap; }
   &__footer { display:flex;align-items:center;justify-content:space-between;color:var(--color-text-muted);font-size:9px; }
