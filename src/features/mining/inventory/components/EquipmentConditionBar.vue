...
<template>
   <div class="condition" :class="{ 'condition--compact': compact }">
      ...
      <div class="condition__track" role="meter" aria-label="Equipment condition" aria-valuemin="0" aria-valuemax="100" :aria-valuenow="value">
         <i class="condition__fill" :class="`condition__fill--${label}`" :style="{ width: `${value}%` }"></i>
      </div>
   </div>
</template>
